...
export default function ItemsPage() {
    ...
    const handleDownload = async () => {
        ...
        await exportToExcel("Items", headers, rows, "Items");
    };
                ...
                <div className="flex gap-2">
                    ...
                    {role === "MDGT" && sortedItems.length > 0 && (
                        <button
                            ...
                            className="flex items-center gap-2 px-3 py-2 text-sm bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-lg hover:from-green-700 hover:to-emerald-700 transition-all shadow-md"
                            title={`Download ${sortedItems.length} items as Excel`}
                        >
                            <Download className="w-4 h-4" />
                            Download
                            <span className="bg-white text-green-700 text-xs font-bold px-1.5 py-0.5 rounded-full leading-none">{sortedItems.length}</span>
                        </button>
                    )}
                </div>
            </div>

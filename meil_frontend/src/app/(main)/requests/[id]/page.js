...
import { fetchRequests, fetchChatMessages, addChatMessage, updateRequest, assignSapItem, assignMaterialGroup, fetchMaterialGroups, fetchItemMasters, markRequestAsRead } from "@/lib/api";
...
export default function RequestDetailPage() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // Trigger sidebar refresh to update unread count
      ...
      try {
        if (token) {
          ...
          if (found && !found.isread) {
            try {
              await markRequestAsRead(token, id);
  ...
  // Scroll to bottom when request loads
  useEffect(() => {
    if (!loading && request) {
  ...
  return (
              ...
              {role === 'MDGT' && request?.type === 'material' && (
                <div className="mt-2 flex items-end gap-2">
                  ...
                  <button
                    ...
                    className="bg-green-600 text-white px-3 py-1.5 text-sm rounded-lg hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap"
              ...
              <div className="space-y-2 mb-3">
                {chatMessages.map((chat, index) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`p-2 rounded-lg max-w-md ${isCurrentUser ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"
                          }`}
                      >
    ...
    focus:ring-2 focus:ring-blue-500 disabled:bg-gray-100 disabled:cursor-not-allowed"
                ...
                <button
                  onClick={handleSend}
                  className="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 text-sm rounded-lg transition-colors flex items-center"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1" viewBox="0 0 20 20" fill="currentColor">
          ...
          <div className="w-full lg:w-80">
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-3 sticky top-3">
              <h2 className="font-semibold text-sm text-gray-700 mb-2 pb-1.5 border-b">Request History</h2>
              <div className="space-y-2 text-xs text-gray-600 mb-3">
                <div className="flex items-start">
                  <div className="w-6 h-6 rounded-full bg-green-100 flex items-center justify-center flex-shrink-0 mr-2">
                ...
                <div className="flex items-start">
                  <div className="w-6 h-6 rounded-full bg-purple-100 flex items-center justify-center flex-shrink-0 mr-2">
                ...
                {isEditing ? (
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="flex gap-1.5">
                      ...
                      <button
                        onClick={() => setIsEditing(false)}
                        className="border px-3 py-1.5 text-sm rounded-lg hover:bg-gray-100"
                      >
                        Cancel
                      </button>
                    </div>

...
export default function MaterialsPage() {
  ...
  const [attributeValidationErrors, setAttributeValidationErrors] = useState({}); // Store validation errors per attribute
  ...
  const validateAttributeValue = (value, validationType) => {
    ...
    if (validation === "alpha") {
      // Remove spaces and check if only alphabetic
      ...
      // Check if numeric (allows decimals and negatives)
  ...
  const handleCustomValueChange = (attrName, customValue) => {
    // Get validation type for this attribute
    const attrConfig = materialAttributes[attrName];
    const validationType = attrConfig?.validation;
    
    // Always update the input field value (for user feedback)
    setCustomAttributeValues(prev => ({
      ...prev,
      [attrName]: customValue
    }));
    
    // Validate if validation type exists
    if (validationType && customValue) {
      const validationResult = validateAttributeValue(customValue, validationType);
      
      // Update validation errors
      setAttributeValidationErrors(prev => ({
        ...prev,
        [attrName]: validationResult.isValid ? null : validationResult.error
      }));
      
      // If invalid, don't save the value to formData.attributes
      if (!validationResult.isValid) {
        // Clear the attribute value if it was previously set
        handleAttributeChange(attrName, "");
        return;
      }
    } else {
      // Clear validation error if no validation or empty value
      setAttributeValidationErrors(prev => {
        const newState = { ...prev };
        if (!customValue) {
          delete newState[attrName];
        } else {
          newState[attrName] = null;
        }
        return newState;
      });
    }
    
    // Only update attribute value if validation passed or no validation is required
  ...
  const handleSaveMaterial = async () => {
    // Validate all custom attribute values before saving
    const validationErrors = {};
    Object.keys(customAttributeValues).forEach(attrName => {
      const value = customAttributeValues[attrName];
      const attrConfig = materialAttributes[attrName];
      const validationType = attrConfig?.validation;
      
      if (validationType && value) {
        const validationResult = validateAttributeValue(value, validationType);
        if (!validationResult.isValid) {
          validationErrors[attrName] = validationResult.error;
        }
      }
    });
    ...
    if (Object.keys(validationErrors).length > 0) {
      setAttributeValidationErrors(prev => ({ ...prev, ...validationErrors }));
      setError("Please fix validation errors in attribute values before saving");
      return;
    }
    
    // Check for existing validation errors
    const hasValidationErrors = Object.values(attributeValidationErrors).some(error => error !== null);
          ...
          <div className="flex flex-col md:flex-row gap-2">
            ...
            {role === "MDGT" && sortedMaterials.length > 0 && (
              ...
              >
                <Download size={14} />
                Download
                <span className="bg-white text-green-700 text-xs font-bold px-1.5 py-0.5 rounded-full leading-none">{sortedMaterials.length}</span>
              </button>
            )}
        ...
        <div className="bg-white rounded-lg shadow-lg overflow-hidden">
          ...
          {loading ? (
            <div className="flex items-center justify-center py-6">
              ...
              <span className="ml-2 text-sm text-gray-600">Loading materials...</span>
            </div>
          ) : (
                    ...
                    {sortedMaterials.length > 0 ? (
                      sortedMaterials.map((material, index) => (
                        ...
                        >
                          ...
                          <td className="px-3 py-2 text-xs text-gray-900">{material.mgrp_code || "-"}</td>
                          <td className="px-3 py-2 text-xs text-gray-900">
                    ...
                    {sortedMaterials.length > 0 ? (
                      sortedMaterials.map((material, index) => (
                        ...
                        >
                          ...
                          <td className="px-3 py-2 text-xs text-gray-900">{material.mat_type_code || "-"}</td>
                          <td className="px-3 py-2 text-xs text-gray-900">{material.item_desc || material.short_name || "-"}</td>
                          <td className="px-3 py-2 text-xs text-gray-900">{material.notes || material.long_name || "-"}</td>
                          <td className="px-3 py-2 text-xs text-gray-900">{material.search_text || "-"}</td>
                          <td className="px-3 py-2 text-xs text-gray-900">{material.created || "-"}</td>
                          <td className="px-3 py-2 text-xs text-gray-900">{material.updated || "-"}</td>
                          <td className="px-3 py-2 text-xs text-gray-900">
                            <div className="flex space-x-1.5">
                              <button
                                onClick={() => handleView(material)}
        ...
        >
          <div className="bg-white rounded-lg shadow-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="border-b border-gray-200">
              <div className="flex">
                ...
                >
                  Attributes
                  {!formData.mgrp_code && (
                    <span className="ml-1.5 text-xs text-gray-400">(Select Material Group first)</span>
                  )}
                </button>
              </div>
            </div>
            ...
            <div className="p-4">
              ...
              {activeTab === "general" && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="block text-xs font-medium text-gray-700 mb-1">Unit of Measure (UOM)</label>
                    <select
                      name="uom"
                      value={formData.uom}
                      onChange={handleInputChange}
                    ...
                    >
                      ...
                      {(materialGroups.find(g => g.mgrp_code === formData.mgrp_code)?.uom_values || [])
                        .filter(u => u && u.trim() !== "")
                        .map(uom => (
                  ...
                  <div className="md:col-span-2">
                    ...
                    <textarea
                      ...
                      rows={3}
                      className="w-full px-3 py-1.5 text-sm border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                      placeholder=" provide  the  Long name"
                      readOnly={Object.keys(formData.attributes || {}).length > 0 && Object.values(formData.attributes || {}).some(v => v)}
                      title={Object.keys(formData.attributes || {}).length > 0 && Object.values(formData.attributes || {}).some(v => v) ? " Long name is auto-populated from attributes" : ""}
                    />
                    {Object.keys(formData.attributes || {}).length > 0 && Object.values(formData.attributes || {}).some(v => v) && (
                      <p className="text-xs text-gray-500 mt-0.5">
                        Long will be automatically generated from attributes when saved
                      </p>
                    )}
                  </div>
                                ...
                                <div className="col-span-3">
                                  <select
                                    ...
                                    onChange={(e) => {
                                      ...
                                      if (selectedValue) {
                                        // Clear custom value and validation error when selecting from dropdown
                                        setCustomAttributeValues(prev => {
                                          const newState = { ...prev };
                                          delete newState[attrName];
                                          return newState;
                                  ...
                                  <div>
                                    <input
                                      ...
                                      onChange={(e) => {
                                        ...
                                        handleCustomValueChange(attrName, newCustomValue);
                                      }}
                                      onBlur={() => {
                                      ...
                                      className={`w-full px-2 py-1.5 text-sm border rounded focus:outline-none focus:ring-1 bg-white ${
                                        attributeValidationErrors[attrName] 
                                    ...
                                    {materialAttributes[attrName]?.validation && !attributeValidationErrors[attrName] && (
                                      <p className="text-xs text-gray-500 mt-0.5">
                                        Validation: {materialAttributes[attrName].validation}
                                      </p>
                                    )}
                                  </div>
                                </div>
            ...
            <div className="flex justify-end space-x-2 p-3 border-t">
              ...
              >
                {saving && <Loader2 className="animate-spin h-3 w-3 mr-1.5" />}
              ...
              <div className="flex">
                ...
                >
                  ...
                  {!viewingMaterial.mgrp_code && (
                    <span className="ml-1.5 text-xs text-gray-400">(No Material Group)</span>
                  )}
                </button>
              </div>
            </div>
            ...
            <div className="p-4">
              ...
              {viewActiveTab === "general" && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="block text-xs font-medium text-gray-700 mb-1">Material Group Long Name</label>
                    <div className="w-full px-3 py-1.5 text-sm border border-gray-300 rounded-lg bg-gray-50 text-gray-900">
                      {(() => {

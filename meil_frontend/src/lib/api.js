...
export const markRequestAsRead = (token, request_id) => {
    const axiosInstance = createAxiosInstance(token);
    return axiosInstance
        .put(`requests/mark-read/${request_id}/`)
        .then(res => res.data);
};

// Favorites API
export const fetchFavorites = (token, type = null) => {
    const axiosInstance = createAxiosInstance(token);
    const url = type ? `favorites/list/?type=${type}` : "favorites/list/";
    return axiosInstance.get(url).then(res => res.data);
};

export const addFavorite = (token, data) => {
    const axiosInstance = createAxiosInstance(token);
    // data can be { mgrp_code } for material groups or { local_item_id } or { sap_item_id } for items
    return axiosInstance.post("favorites/add/", data).then(res => res.data);
};

export const removeFavorite = (token, data) => {
    const axiosInstance = createAxiosInstance(token);
    // data can be { mgrp_code } for material groups or { local_item_id } or { sap_item_id } for items
    return axiosInstance.post("favorites/remove/", data).then(res => res.data);
};
...
// Share Materials API
export const shareMaterial = (token, data) => {
    const axiosInstance = createAxiosInstance(token);
    // data should be { local_item_id or sap_item_id, shared_with: [emp_id1, emp_id2, ...] }
    return axiosInstance.post("favorites/share/", data).then(res => res.data);
};

export const fetchSharedMaterials = (token) => {
    const axiosInstance = createAxiosInstance(token);
    return axiosInstance.get("favorites/shared/").then(res => res.data);
};

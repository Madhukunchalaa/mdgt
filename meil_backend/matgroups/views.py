...
def create_matgroup(request):
    if request.method == "POST":
        try:
            ...
            uom_values = data.get("uom_values", [])
            ...
            matgroup = MatGroup.objects.create(
                ...
                uom_values=uom_values if isinstance(uom_values, list) else [],
                notes=notes,
                createdby=createdby,
                updatedby=createdby
            )
            ...
            response_data = {
                ...
                "uom_values": matgroup.uom_values or [],
                "notes": matgroup.notes,
                "sgrp_code": matgroup.sgrp_code.sgrp_code if matgroup.sgrp_code else None,
        ...
        try:
            ...
            for mg in matgroups:
                response_data.append({
                    ...
                    "uom_values": mg.uom_values or [],
                    "notes": mg.notes,
                    "sgrp_code": mg.sgrp_code.sgrp_code if mg.sgrp_code else None,
...
def update_matgroup(request, mgrp_code):
    if request.method == "PUT":
        try:
            ...
            if "uom_values" in data:
                matgroup.uom_values = data["uom_values"] if isinstance(data["uom_values"], list) else []
